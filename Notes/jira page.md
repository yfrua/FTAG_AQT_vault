Feirui will derive the light jet mistag calibration of the taggers **GN3PlowMuons or GN3EPCLV01** at high jet pT. The conventional light mistag calibration is done with the negative tag method on a Z+jets selection. However, this selection yields jets with a rapidity falling jet pT spectrum, calibrations can only be done for jets up to pT=300 GeV. A [previous AQP task](Ellen's%20presentations.md) applied for the first time the current light jet mistag calibration strategy on a high pT jet sample in Rel.21 using a di-jet selection. This calibration uses a similar strategy for the scale factor extraction but a different selection. 
